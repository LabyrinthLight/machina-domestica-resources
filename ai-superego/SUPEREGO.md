...
- Obligation on a cycle that must be nagged if missed (rent, salary, bill payments, homeschool block) → Recurring tasks.
- Homeschool Olivia is a recurring task, not a habit. Planned skip ≠ miss. Unplanned miss → add Open loop "Makeup homeschool YYYY-MM-DD" (overdue until done or struck). "Strike that day" / "don't make it up" → delete that makeup loop only; do not kill the series.
